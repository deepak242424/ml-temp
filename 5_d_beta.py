...
def calc_class_priors(class_0_fre, class_1_fre, vocabulary):
    ...
    alp = 1
    bet = 10
    alpha = alp*np.ones(24748)
    beta = bet*np.ones(24748)
    N1 = np.ones(24748)*(np.sum(spam_array))
    N2 = np.ones(24748)*(np.sum(legit_array))
    para_cls_0_a =  alpha + spam_array
    para_cls_0_b =  beta + N1-spam_array
    prior_cl0 =  np.divide((alpha + spam_array),(alpha + beta + N1))
    para_cls_1_a = alpha + legit_array
    para_cls_1_b = beta + N2-legit_array
    prior_cl1 =  np.divide((alpha + legit_array), (alpha + beta + N2))

    prob_cls_0 = alp/float(alp+bet)
    prob_cls_1 = bet/float(alp+bet)

    return prior_cl0, prior_cl1, prob_cls_0, prob_cls_1
...
def train_function(dataset, fol_list):
    ...
    spam_priors, legit_priors, prob_cls_0, prob_cls_1 = calc_class_priors(spams_fre, legits_fre, vocabulary)
...
def plot_five_foldsPR():
    ...
    plt.title('PR Curve for Spam Class in Beta')
    plt.show()
plot_five_foldsPR()
